...
namespace Laba3
{
    public partial class ChartOne : Form
    {
        ...
        {
            var regressionCoefficients = Fit.Line(matrix.Y, matrix.X1);

            double slope = regressionCoefficients.Item2;
            double intercept = regressionCoefficients.Item1;

            string equation1 = $"X1 = {slope} * Y + {intercept}";
            ...
            System.Windows.Forms.DataVisualization.Charting.Series series2 = new System.Windows.Forms.DataVisualization.Charting.Series("X1");
            ...
            for (int i = 0; i < matrix.Y.Length; i++)
            {
                series2.Points.AddXY(matrix.Y[i], matrix.X1[i]);
            }
